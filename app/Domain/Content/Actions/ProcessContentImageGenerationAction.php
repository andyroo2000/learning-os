...
final class ProcessContentImageGenerationAction
{
    ...
    /** @return null|array{sourceText: string, targetLanguage: string, imageCount: int, sentences: list<array{id: string, text: string}>} */
    ...
    private function releaseClaim(string $jobId): void
    {
        DB::transaction(function () use ($jobId): void {
            ContentSourceLock::acquireConvoLab(DB::connection());
            $job = ContentImageGenerationJob::query()->whereKey($jobId)->lockForUpdate()->first();
            if ($job === null || $job->state !== ContentImageGeneration::STATE_ACTIVE) {
                return;
            }
    ...
    private function complete(string $jobId, array $plannedImages): void
    {
        DB::transaction(function () use ($jobId, $plannedImages): void {
            ContentSourceLock::acquireConvoLab(DB::connection());
            $job = ContentImageGenerationJob::query()->whereKey($jobId)->lockForUpdate()->first();
            if ($job === null || $job->state !== ContentImageGeneration::STATE_ACTIVE) {
                return;
