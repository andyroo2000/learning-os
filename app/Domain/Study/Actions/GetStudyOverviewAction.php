...
class GetStudyOverviewAction
{
    ...
    private function cardMetrics(
        ...
        $statuses = $this->cardMetricStatuses();
        $timestamps = [
        ...
        $row = $this->cardsQuery->forUser($userId, $courseId, $deckId)
            ->selectRaw(
                $this->cardMetricsSql($statuses),
                $this->cardMetricBindings($userId, $statuses, $timestamps),
            )
            ->first();

        return $this->normalizeCardMetrics($row);
    }

    /**
     * @param  array{active_due: list<string>, learning: list<string>, suspended: list<string>}  $statuses
     */
    private function cardMetricsSql(array $statuses): string
    {
        $activeDueStatusPlaceholders = $this->statusPlaceholders($statuses['active_due'], 'active due statuses');
        $learningStatusPlaceholders = $this->statusPlaceholders($statuses['learning'], 'learning statuses');
        $suspendedStatusPlaceholders = $this->statusPlaceholders($statuses['suspended'], 'suspended statuses');
        ...
        return <<<SQL
            ...
            COALESCE((SELECT MAX(study_settings.new_cards_per_day) FROM study_settings WHERE study_settings.user_id = ?), ?) AS new_cards_per_day,
            COALESCE((SELECT MAX(study_settings.lesson_batch_size) FROM study_settings WHERE study_settings.user_id = ?), ?) AS lesson_batch_size,
            COALESCE((SELECT MAX(study_settings.review_time_budget_minutes) FROM study_settings WHERE study_settings.user_id = ?), ?) AS review_time_budget_minutes,
            COALESCE((SELECT MAX(study_settings.standard_lane_weight) FROM study_settings WHERE study_settings.user_id = ?), ?) AS standard_lane_weight,
            COALESCE((SELECT MAX(study_settings.lesson_followup_lane_weight) FROM study_settings WHERE study_settings.user_id = ?), ?) AS lesson_followup_lane_weight,
            COALESCE((SELECT MAX(study_settings.wanikani_lane_weight) FROM study_settings WHERE study_settings.user_id = ?), ?) AS wanikani_lane_weight,
            (
    ...
    /**
     * @param  array{active_due: list<string>, learning: list<string>, suspended: list<string>}  $statuses
     * @param  array{now: string, day_start: string, day_end: string}  $timestamps
     * @return list<int|string>
     */
    private function cardMetricBindings(int $userId, array $statuses, array $timestamps): array
    {
        $availableVariantStatus = VocabVariantStatus::Available->value;
        $nowFormatted = $timestamps['now'];
    ...
    /**
     * @return array{active_due: list<string>, learning: list<string>, suspended: list<string>}
     */
    private function cardMetricStatuses(): array
    {
        return [
            'active_due' => $this->cardsQuery->activeDueStatuses(),
            'learning' => $this->learningStatuses(),
            'suspended' => $this->suspendedStatuses(),
        ];
    }

    /** @return array<string, int|string|null> */
    private function normalizeCardMetrics(?object $row): array
    {
