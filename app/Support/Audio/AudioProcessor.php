...
class AudioProcessor
{
    ...
    public function normalize(string $inputPath, string $outputPath): void
    {
        $this->run([
            ...
            '-af', 'dynaudnorm=f=150:g=15:p=0.9:m=10', '-ar', '44100', '-ac', '2',
    ...
    public function silence(float $seconds, string $outputPath): void
    {
        $this->run([
            ...
            '-i', 'anullsrc=r=44100:cl=stereo', '-t', $this->decimal($seconds),
            '-c:a', 'libmp3lame', '-b:a', '128k', $outputPath,
        ]);
        $this->assertOutput($outputPath);
    }
    ...
    {
        if ($segmentPaths === []) {
            throw new RuntimeException('Audio track has no audio segments.');
        }
        ...
        if (file_put_contents($listPath, implode("\n", $lines)."\n", LOCK_EX) === false) {
            throw new RuntimeException('Audio concat manifest could not be written.');
        }

        $this->run([
            'ffmpeg', '-hide_banner', '-loglevel', 'error', '-y', '-f', 'concat',
            '-safe', '0', '-i', $listPath, '-af', implode(',', [
                'highpass=f=80',
    ...
    public function truncate(string $inputPath, float $seconds, string $outputPath): void
    {
        $this->run([
            'ffmpeg', '-hide_banner', '-loglevel', 'error', '-y', '-i', $inputPath,
            '-t', $this->decimal($seconds), '-ar', '44100', '-ac', '2',
            '-c:a', 'libmp3lame', '-b:a', '128k', $outputPath,
        ]);
        $this->assertOutput($outputPath);
    }
    ...
    public function duration(string $path): float
    {
        ...
        if (! is_float($duration) || ! is_finite($duration) || $duration <= 0) {
            throw new RuntimeException('Audio segment duration could not be determined.');
        }
    ...
    private function assertOutput(string $path): void
    {
        ...
        if (! is_file($path) || filesize($path) === 0) {
            throw new RuntimeException('Audio processing produced no output.');
        }
    }
