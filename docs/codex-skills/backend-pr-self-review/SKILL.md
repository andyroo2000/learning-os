...
- A payload comparison uses `assertSame()` or `assertEquals()` without matching the actual cast shape: arrays/scalars can use strict equality, fresh value-object instances need structural equality or a documented identity/memoization contract, and `assertEquals()` needs type-sensitive spot coverage for numeric-string-prone fields.
- An expected sync/resource payload is built from a stale in-memory model, or from a model missing relations that the payload builder traverses, after the action/job changed persisted status, error fields, deletion timestamps, generated outputs, relation-derived IDs, or other fields the payload builder reads.
