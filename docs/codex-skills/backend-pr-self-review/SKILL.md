...
- Rate limiter changes lack explicit route wiring, stable key fallback coverage, default-limit coverage, and isolated test buckets or cleanup for temporary overrides. New named limiters that are meant to have separate quotas use a raw `Limit::by()` key that could collide with sibling limiter keys; named `RateLimiter::for()` handlers do not isolate buckets unless the returned key includes an operation/limiter prefix.
