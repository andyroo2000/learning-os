...
- A canonical model-to-payload comparison replaces the only fixed expected-value assertion for a transformation, allowing the model and feed to be wrong in the same way.
- A sync feed assertion checks only one payload field, or identifies entries by array position/checkpoint order when stable business keys such as `user_id`, `operation`, or the full `domain`/`resource_type`/`resource_id` tuple are available.
...
- A query-count optimization replaces a settings/helper lookup with a defaulting SQL aggregate or scalar subquery but tests only the missing-row/default path, leaving explicit persisted settings unproven.
...
- A timestamp preservation or idempotency test uses null-safe timestamp access, such as `?->toJSON()`, for a freshly persisted model timestamp that should be non-null, or mixes ordinary and null-safe timestamp access without making the nullability contract explicit.
