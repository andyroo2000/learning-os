...
- A test assigns `$queries` or another assertion input only inside `finally` and consumes it after the block without an outer initialization, leaving static analysis and future edits with a maybe-undefined variable shape.
