...
- Test key fallbacks for missing user/email, empty strings, whitespace-only values, non-string payloads, missing or empty IP, and sentinel text used as a real value. If tests need key internals, expose the smallest stable key-format method rather than duplicating the formula in feature tests; when that method is public only as a testable contract, mark it `@internal` or comment that route code should use the named limiter methods.
