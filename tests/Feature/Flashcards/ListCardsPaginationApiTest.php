...
class ListCardsPaginationApiTest extends TestCase
{
    ...
    {
        $this->assertPaginationFilterPersists($this->coursePaginationFixture());
    }
    ...
    public function test_it_preserves_card_type_filter_when_following_a_cursor(): void
    {
        $this->assertPaginationFilterPersists($this->cardAttributePaginationFixture([
            'key' => 'card_type',
            'value' => 'production',
    ...
    {
        $this->assertPaginationFilterPersists($this->deckPaginationFixture());
    }

    public function test_it_preserves_search_query_when_following_a_cursor(): void
    {
        $this->assertPaginationFilterPersists($this->cardAttributePaginationFixture([
            'key' => 'q',
            'value' => 'photo',
    ...
    /** @return array{query: string, key: string, value: string, first_id: string, second_id: string, excluded_id: string} */
    private function coursePaginationFixture(): array
    {
        $user = $this->signIn();
        $course = Course::factory()->for($user)->create();
        $otherCourse = Course::factory()->for($user)->create();

        return $this->cardPaginationFixture(
            $this->deckFor($user, ['course_id' => $course->id]),
            $this->deckFor($user, ['course_id' => $otherCourse->id]),
            ['key' => 'course_id', 'value' => $course->id],
        );
    }

    /** @return array{query: string, key: string, value: string, first_id: string, second_id: string, excluded_id: string} */
    private function deckPaginationFixture(): array
    {
        $user = $this->signIn();
        $deck = $this->deckFor($user);
    ...
    private function studyStatusPaginationFixture(): array
    {
        ...
        $secondCard = $this->cardWithStudyStatus($deck, CardStudyStatus::Review, [
            'created_at' => now()->subMinute(),
            'updated_at' => now()->subMinute(),
        ]);
        $excludedCard = $this->cardWithStudyStatus($deck, CardStudyStatus::New, [
            'created_at' => now()->subSeconds(30),
            'updated_at' => now()->subSeconds(30),
        ]);

        return $this->paginationFixture('study_status', 'review', [$firstCard, $secondCard, $excludedCard]);
    }
    ...
    private function cardAttributePaginationFixture(array $filter): array
    {
        ...
        return $this->cardPaginationFixture($deck, $deck, $filter);
    }
     ...
     * @return array{query: string, key: string, value: string, first_id: string, second_id: string, excluded_id: string}
     */
    private function cardPaginationFixture(Deck $deck, Deck $excludedDeck, array $filter): array
    {
        ...
        $excludedCard = Card::factory()->for($excludedDeck)->create(($filter['excluded'] ?? []) + [
            'created_at' => now()->subSeconds(30),
            'updated_at' => now()->subSeconds(30),
        ]);

        return $this->paginationFixture($filter['key'], $filter['value'], [$firstCard, $secondCard, $excludedCard]);
    }
     ...
     * @return array{query: string, key: string, value: string, first_id: string, second_id: string, excluded_id: string}
     */
    private function paginationFixture(string $key, string $value, array $cards): array
    {
        [$firstCard, $secondCard, $excludedCard] = $cards;
    ...
    /** @param array{query: string, key: string, value: string, first_id: string, second_id: string, excluded_id: string} $fixture */
    private function assertPaginationFilterPersists(array $fixture): void
    {
        $firstPage = $this->getJson("/api/cards?{$fixture['query']}&per_page=1");
