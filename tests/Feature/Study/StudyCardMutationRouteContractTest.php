...
class StudyCardMutationRouteContractTest extends TestCase
{
    private const CARD_ID_PATTERN = '(?:[0-9A-HJKMNP-TV-Za-hjkmnp-tv-z]{26}|[0-9a-fA-F]{8}(?:-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12})';

    public function test_study_card_mutation_routes_preserve_registration_order_names_actions_middleware_and_constraints(): void
    {
        $actions = [
            'StoreStudyCardController',
            'DeleteStudyCardController',
            'PerformStudyCardActionController',
            'UpdateStudyCardController',
        ];
        ...
        $this->assertSame([
            ...
            $this->expectedRoute(
                ...
                'DeleteStudyCardController',
                'throttle:study-card-delete',
                ['cardId' => self::CARD_ID_PATTERN],
            ),
            $this->expectedRoute(
                'POST',
                'api/study/cards/{cardId}/actions',
                'PerformStudyCardActionController',
                'throttle:study-card-action',
                ['cardId' => self::CARD_ID_PATTERN],
            ),
            $this->expectedRoute(
                'PATCH',
    ...
    public function test_study_card_mutation_routes_remain_inside_the_network_limit_at_their_original_global_boundaries(): void
    {
